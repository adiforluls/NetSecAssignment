...
def encrypt():
    # only encrypt single blocks of 64 bits
    
    msg = ip1.get("1.0",'end-1c')
    msg = int(msg)
    key = ip3.get("1.0",'end-1c')
    key = int(key)
    n = ip2.get("1.0",'end-1c')
    n = int(n)
    op1.delete("1.0",'end-1c')
    ...
    # apply thr round function n times in following scheme (feistel cipher):
    L_last = L0
    R_last = R0
...
def decrypt():
    #similar process as encryption
    
    # permutate by table PC1
    msg = op1.get("1.0",'end-1c')
    msg = int(msg)
    key = op3.get("1.0",'end-1c')
    key = int(key)
    n = op2.get("1.0",'end-1c')
    n = int(n)
    ip1.delete("1.0",'end-1c')
    key = permutation_by_table(key, 64, PC1) # 64bit -> PC1 -> 56bit
...
frame2 = Frame(root)
frame2.pack()
ip2 = Text(frame2, height = 1, width = 5)
ip2.pack(side=LEFT, fill=BOTH, expand=YES)

frame3 = Frame(root)
frame3.pack()
ip3 = Text(frame3, height = 2, width=100)
ip3.pack(side=LEFT, fill=BOTH, expand=YES)

frame4 = Frame(root)
frame4.pack()
enc = Button(frame4, text="Encrypt", command = encrypt)
dec = Button(frame4, text="Decrypt", command = decrypt)
enc.pack(side = LEFT)
dec.pack(side = RIGHT)

frame5 = Frame(root)
frame5.pack()
op1 = Text(frame5, height = 2, width = 100)
op1.pack(side=RIGHT, fill=BOTH, expand=YES)

frame6 = Frame(root)
frame6.pack()
op2 = Text(frame6, height=1, width = 5)
op2.pack(side=RIGHT, fill=BOTH, expand=YES)

frame7 = Frame(root)
frame7.pack()
op3 = Text(frame7,height=2,width=100)
op3.pack(side=RIGHT, fill=BOTH, expand=YES)
